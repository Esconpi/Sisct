...
namespace Escon.SisctNET.Web.Controllers
{
    public class NoteInventoryExitController : ControllerBaseSisctNET
    {
        ...
        public IActionResult GetAll(int draw, int start)
        {
            ...
            {
                List<NoteInventoryExit> notes = new List<NoteInventoryExit>();

                var filter = Helpers.CharacterEspecials.RemoveDiacritics(Request.Query["search[value]"].ToString());

                List<NoteInventoryExit> notesTemp = new List<NoteInventoryExit>();
            ...
            for (int i = notes.Count - 1; i >= 0; i--)
            {
                ...
                if (nota == null)
                {
                    try
                    {
                        ...
                        Model.NoteInventoryExit note = new Model.NoteInventoryExit();
                    ...
                    nota = _service.FindByNote(notes[i][0]["chave"]);
                }
                    ...
                    if (notes[i][j].ContainsKey("baseCalc"))
                    {
                        ...
                        if (productImport == null)
                        {
                            ...
                            try
                            {
                                ...
                                prod.NoteInventoryExitId = nota.Id;
